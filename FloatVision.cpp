...
// NOTE: HTMLファイル本体はサイズ制限回避のため常に Navigate(file://...) を優先する。
bool LoadHtmlFromFile(const wchar_t* path)
{
    if (!path || !*path)
    {
...
void ApplyWebViewContentZoom()
{
    ...
    script << L"(() => {"
           ...
           << L"const body=document.body;"
           << L"if(!root||!body){return;}"
           << L"root.style.setProperty('--floatvision-content-zoom', String(zoom));"
           << L"const styleId='floatvision-content-zoom-style';"
           << L"const wrapperId='floatvision-content-zoom-root';"
           << L"let wrapper=document.getElementById(wrapperId);"
           << L"if(!(wrapper instanceof HTMLDivElement)){"
           << L"wrapper=document.createElement('div');wrapper.id=wrapperId;"
           << L"while(body.firstChild){wrapper.appendChild(body.firstChild);}"
           << L"body.appendChild(wrapper);"
           << L"}"
           ...
           << L"style.textContent='body{margin:0 !important;}#floatvision-content-zoom-root{zoom:var(--floatvision-content-zoom,1) !important;transform-origin:top left;}';"
